...
void NetworkWidget::doLayout(Renderer& renderer, float containerWidth, float containerHeight) {
  ...
  if (m_isVertical && labelVisible) {
    ...
    m_label->setPosition(
        std::round((w - m_label->width()) * 0.5f),
        (vpnVisible ? m_vpnGlyph->height() : 0.0f) + icon->height()
    );
    rootNode->setSize(w, (vpnVisible ? m_vpnGlyph->height() : 0.0f) + icon->height() + m_label->height());
  } else {
    const float h =
        labelVisible ? std::max({(vpnVisible ? m_vpnGlyph->height() : 0.0f), icon->height(), m_label->height()})
